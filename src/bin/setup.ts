...
const userMod = require(path.join(__dirname, "../models/user_model"));
...
async function main() {
	try {
		...
		await cp_replace("../../templates/app.env.sample", path.join(destination_path, ".env"), opts, "{", "}");
		await cp("../../templates/app-server.js", path.join(destination_path, "bin/server.js"));
		...
		try {
			process.env.MONGO_CONNECTION_STRING = opts.connection_string;
...
function write(filePath: string, str: string, mode?: number) {
	fs.writeFileSync(filePath, str, mode !== undefined ? { mode } : undefined);
